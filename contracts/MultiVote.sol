...
contract SimpleVote {
    ...
    constructor(address _bondManager, uint256 _options, uint256 _deadline, bytes32 _descriptionDigest, address _asset) public {
    ...
    function processVote(uint256[] memory optionIds, uint256[] memory amounts) public {
        assert(optionIds.length == amounts.length);                 // ensure arguments are valid
        ...
        for (uint256 i = 0; i < optionIds.length; i++) {
            ...
            if (i > 0) { assert(optionIds[i] > optionIds[i-1]); }   // optionIds must be in order, to prevent duplicates

            uint256 newTotal = total + amounts[i];
            assert(newTotal > total);                               // prevent overflow
            total = newTotal;

            tallies[optionIds[i]] += amounts[i];                    // add support to option (no need for safe math since total must later be less than alloted, which is in real wei)
            records[msg.sender] += amounts[i];                      // record that account has voted on this proposal, and how much, (not the direction)

            if (tallies[optionIds[i]] >= tallies[leadingId]) {
                leadingId = optionIds[i];                           // update the local leading option if it has been overtaken
            }

            emit VoteCounted(msg.sender, optionIds[i], amounts[i]);
        }
